...
const (
	...
	ObjectReplicationAll                 EventName = "s3:Replication:*"
	ObjectReplicationPut                 EventName = "s3:Replication:Put"
	ObjectReplicationDelete              EventName = "s3:Replication:Delete"
	ObjectReplicationFailed              EventName = "s3:Replication:OperationFailedReplication"
	ObjectReplicationComplete            EventName = "s3:Replication:OperationCompletedReplication"
	ObjectReplicationNotTracked          EventName = "s3:Replication:OperationNotTracked"
	ObjectReplicationMissedThreshold     EventName = "s3:Replication:OperationMissedThreshold"
	ObjectReplicationAfterThreshold      EventName = "s3:Replication:OperationReplicatedAfterThreshold"
	...
	LifecycleAll                         EventName = "s3:Lifecycle:*"
	LifecyclePut                         EventName = "s3:Lifecycle:Put"
	LifecycleDelete                      EventName = "s3:Lifecycle:Delete"
)
...
func (e EventName) Validate() error {
	...
	case BucketRemoved,
		...
		LifecyclePut,
		LifecycleDelete:
		if _, ok := unsupportedEvents[e]; ok {
			return fmt.Errorf("unsupported event '%s' now", e)
		}
		return nil
	default:
	...
	if len(nc.Filter.FilterRules) > 0 {
		...
		for _, rule := range nc.Filter.FilterRules {
			...
			case "prefix", "suffix":
				...
				if !utf8.ValidString(rule.Value) || len(rule.Value) > 1024 {
					return fmt.Errorf("invalid filter rule value '%s'", rule.Value)
				}
...
func (na *NotificationARN) Validate(region string, notifier *EventNotifier) error {
	if notifier == nil {
	...
	if notifier.GetNotifier(na.NotifierID()) == nil {
...
func (nq *NotificationQueue) Validate(region string, notifier *EventNotifier) error {
	if err := nq.ARN.Validate(region, notifier); err != nil {
		return err
	}
...
func (c *NotificationConfig) Validate(region string, notifier *EventNotifier) error {
	...
	if len(c.Queues) > 0 {
		for i, q1 := range c.Queues[:len(c.Queues)-1] {
			if err := q1.Validate(region, notifier); err != nil {
				return err
			}
			q1.ID = ""
		...
		return c.Queues[len(c.Queues)-1].Validate(region, notifier)
	}
...
func ParseNotificationConfig(data []byte, region string, notifier *EventNotifier) (*NotificationConfig, error) {
	var cfg NotificationConfig
	if err := xml.Unmarshal(data, &cfg); err != nil {
		return nil, err
	}

	if err := cfg.Validate(region, notifier); err != nil {
		return nil, err
	}
